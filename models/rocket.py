...
def generate_kernels(input_length, num_kernels, num_channels=1):
    ...
    # exponential
    num_channel_indices = (
        2 ** np.random.uniform(0, np.log2(num_channels + 1), num_kernels)).astype(np.int32)
    ...
    for i in range(num_kernels):
        ...
        channel_indices[a1:b1] = np.random.choice(
            np.arange(0, num_channels), num_channel_indices[i], replace=False)

        biases[i] = np.random.uniform(-1, 1)

        dilation = 2 ** np.random.uniform(0,
                                          np.log2((input_length - 1) // (lengths[i] - 1)))
    ...
    for i in range(0, output_length, stride):
        ...
        for j in range(length):
            for k in range(num_channel_indices):
                _sum += weights[channel_indices[k], j] * \
                    X[i + (j * dilation), channel_indices[k]]
...
def apply_kernels(X, kernels, stride=1):
    ...
    _X = np.zeros((num_examples, num_kernels * 2),
                  dtype=np.float32)  # 2 features per kernel
...
class RocketRegressor(TimeSeriesRegressor):
    ...
    def fit(self,
        ...
        self.kernels = generate_kernels(
            x_train.shape[1], self.n_kernels, x_train.shape[2])
        ...
        print('[{}] Training done!, took {}s'.format(
