...
do
    ...
    do
        ...
        do
            # BASIC
            python main.py --comment "${DATA}_CONVIT_ONLYRELPOS_TUNING_LR=${LR}_SMOOTH=${LAM}" \
                --seed $SEED --name "${DATA}_CONVIT_ONLYRELPOS_TUNING_LR=${LR}_SMOOTH=${LAM}" \
                --records_file "${DATA}_CONVIT_ONLYRELPOS_TUNING.xls" \
