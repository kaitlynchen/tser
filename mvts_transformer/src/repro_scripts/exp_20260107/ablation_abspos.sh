...
for WHERE in start_concat start_add before_pool_concat
do
    for ABSPOS in learnable_sin_init learnable
    do
        OUTPUT_FILE="${DATA}_OVERSMOOTHING_BASELINE6_ABSPOS=${ABSPOS}_WHERE=${WHERE}"
