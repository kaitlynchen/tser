...
OUTPUT_FILE="${DATA}_OVERSMOOTHING_NEWBASELINE_NEUTRENO"

for LR in 1e-3 1e-2
do
    for NEUTRENO in 0 1e-5 1e-3 1e-2 1e-1 1 100
    do
        ...
        do
            ...
            do
                for SEED in 0
                do
                    PARAM_STR="LR=${LR}_NEUTRENO=${NEUTRENO}_SEED=${SEED}"

                    # Centered attn
                    python main.py --output_dir "./output/oversmoothing_exp" \
                        ...
                        --pool seqpool_multihead --lambda_neutreno $NEUTRENO
                done
            done
        done
    done
