...
# ============================= USAGE ==============================
# Ablation: test different pooling options.
...
# OUTPUT FILE
OUTPUT_FILE="${DATA}_ABLATION_POOL"

for POOL in seqpool_multihead average linear
do
    for SEED in 0 1 2
    do
        # BASIC
        ...
        python main.py --seed $SEED --name "${OUTPUT_FILE}_${PARAM_STR}" \
            ...
            --pool $POOL --reg_lambda_pool $SMOOTH --reg_lambda $SMOOTH --l1_reg $L1
    done
done
