...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * window_size[0] - 1), num_heads))  # 2*Wt-1, nH
    ...
    def forward(self, x, mask=None):
        ...
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
            self.window_size[0], self.window_size[0], -1)  # Wt, Wt, nH
    ...
    def forward(self, x):
        ...
        if self.shift_size > 0:
            if not self.fused_window_process:
                ...
                x = WindowProcessReverse.apply(attn_windows, B, T, C, self.shift_size, self.window_size)
        else:
            shifted_x = window_reverse(attn_windows, self.window_size, T)  # B T C
            x = shifted_x
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=1, embed_dim=96, norm_layer=None):
        ...
        self.proj = nn.Conv1d(in_chans, embed_dim, kernel_size=patch_size[0], stride=patch_size[0])
