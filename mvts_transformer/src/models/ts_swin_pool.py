...
try:
    ...
    kernel_path = os.path.abspath(os.path.join("~/Swin-Transformers/"))
...
def model_factory(config, data):
    ...
    if (task == "classification") or (task == "regression"):
        ...
        if config["model"] == "swin_pool":
            ...
            if config["data_window_len"] == None:
                window_size = 6
...
def pad_data(data, length, pad_amount):
    ...
    for i in range(int(data.shape[0] / length)):
        zeros = pd.DataFrame(
            [[0] * len(data.columns.values)] * pad_amount,
            columns=list(data.columns.values),
            dtype=np.float64,
        )
        padded_df = pd.concat(
            [padded_df, data.iloc[i * length : (i + 1) * length], zeros],
            ignore_index=True,
        )
...
def window_reverse(windows, window_size, T):
    """
    Args:
        windows: (num_windows*B, window_size, C)
        window_size (int): Window size
    ...
    Returns:
        x: (B, T, C)
    """
    B = int(windows.shape[0] / (T / window_size))
    x = windows.view(B, T // window_size, window_size, -1)
...
class AttentionPool2d(nn.Module):
    ...
    def __init__(self, ni, out=1, bias=True, norm=nn.LayerNorm):
        super().__init__()
        self.norm = norm(ni)
    ...
    def forward(self, x, cls_q):
        ...
        k, v = (
            self.vk(x).reshape(B, N, 2, C).permute(2, 0, 1, 3).chunk(2, 0)
        )  # k, v: [1, B, N, C]
...
class WindowAttention(nn.Module):
    ...
    def __init__(
        ...
        relative_coords += self.window_size[0] - 1  # shift to start from 0
    ...
    def forward(self, x, mask=None):
        ...
        Args:
            ...
            mask: (0/-inf) mask with shape of (num_windows, Wt, Wt) or None
        """
        B_, N, C = x.shape
        qkv = (
            self.qkv(x)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.shift_size > 0:
            ...
            T = self.input_resolution
            timesteps = T[0]
            img_mask = torch.zeros((1, timesteps, 1))  # 1, T, 1
            t_slices = (
    ...
    def forward(self, x):
        ...
        if self.shift_size > 0:
            if not self.fused_window_process:
                ...
                raise NotImplementedError("windowprocess.apply not implemented yet")
        ...
        if self.shift_size > 0:
            if not self.fused_window_process:
                ...
                x = WindowProcessReverse.apply(
                    attn_windows, B, T, C, self.shift_size, self.window_size
                )
        else:
            shifted_x = window_reverse(attn_windows, self.window_size, T)  # B T C
            x = shifted_x
...
class MaxPoolMerging(nn.Module):
    r"""MaxPooling Merging Layer.
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        self, img_size=224, patch_size=4, in_chans=1, embed_dim=96, norm_layer=None
    ):
        super().__init__()
    ...
    def forward(self, x):
        ...
        assert (
            T == self.img_size[0]
        ), f"Input time series size ({T}) doesn't match model ({self.img_size[0]})."
        x = self.proj(x).transpose(1, 2)  # B Pt C
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.class_token = nn.Parameter(
            torch.zeros(1, 1, int(embed_dim * 2 ** (self.num_layers - 1)))
        )
...
class PretrainedSwinTransformer(nn.Module):
    ...
    def forward_features(self, x, padding_masks):
        ...
        padding_masks = padding_masks.view(B, L, -1)
        padding_masks = padding_masks.repeat(1, 1, C // padding_masks.shape[2])
        w = padding_masks.type_as(mask_tokens)
    ...
    def forward(self, x, padding_masks):
        x = self.forward_features(x, padding_masks)
        x = self.head(x)
        return x
