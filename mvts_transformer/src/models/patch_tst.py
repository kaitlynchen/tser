...
from models.PatchTST.patch_tst_revin import RevIN
from models.PatchTST.patch_tst_layers import *
...
def model_factory(config, data):
    ...
    if (task == "classification") or (task == "regression"):
        ...
        if config['model'] == 'patch':
            # context_window = lookback window, target_window = how many timesteps to predict for in the future
            # try setting stride to patch_length initially, make sure they divide the number of timesteps evenly
            return PatchTST_backbone(feat_dim, max_seq_len, config['data_window_len'], 
                                     config['patch_length'], config['stride'], num_labels, max_seq_len,
...
class PatchTST_backbone(nn.Module):
    def __init__(self, c_in:int, context_window:int, target_window:int, patch_len:int, stride:int, num_classes:int, max_seq_len:Optional[int]=1024, 
        ...
        # Final linear layer
        self.output_layer = nn.Linear(c_in * context_window, num_classes)
    ...
    def forward(self, z):                                                                   # z: [bs x nvars x seq_len]
        ...
        z = torch.reshape(z, (z.shape[0], z.shape[1] * z.shape[2])) 
