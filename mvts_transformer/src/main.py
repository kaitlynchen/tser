...
def main(config):
    ...
    for epoch in tqdm(range(start_epoch + 1, config["epochs"] + 1), desc="Training Epoch", position=0, leave=True):
        ...
        train_losses_input_grad.append(input_grad_loss)
    ...
    if plot_losses:
        ...
        if config["lambda_input_grad"] > 0:
            plt.plot(train_epochs[1:], train_losses_input_grad[1:], label="Input gradient loss")
