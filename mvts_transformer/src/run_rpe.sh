...
do
    ...
    #     do
    #         # Usage: ./run.sh AppliancesEnergy, from mvts_transformer/src directory. Run "mkdir output" first.
    #         python main.py --comment "ClimaX with attention smoothing on ${DATA}" \
    #             --seed $SEED --name ClimaX_smooth_${DATA}_patch=8_stride=2_lam=${LAM}_seed=${SEED}_sinusoidal \
    #             --records_file climax_smooth_${DATA}_hyperparameter_test.xls \
    #             --data_dir /mnt/beegfs/bulk/mirror/jyf6/datasets/TSER/$DATA/ --data_class tsra \
    #             --pattern TRAIN --val_pattern TEST --epochs 500 --lr 0.001 \
    #             --num_layers 3 --num_heads 16 --d_model 128 --dim_feedforward 512 \
    #             --optimizer RAdam  --pos_encoding learnable --task regression \
    #             --model climax_smooth --patch_length 8 --stride 2 --reg_lambda $LAM --smooth_attention
    #     done
    # done


#AppliancesEnergy BenzeneConcentration BeijingPM10Quality BeijingPM25Quality IEEEPPG LiveFuelMoistureContent
